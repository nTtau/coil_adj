...
# Calculates the magnetic field from one coil using its properties
    ...
    for col in file:
        ...
        #Read in values
#            print(coil_counter)
        Nr.append(int(float(col['R_turns'])))
        Nz.append(int(float(col['Z_turns'])))
 ...
 #   print("coil x,y,z:")
  #  print(data_array[c_n,6],data_array[c_n,7],data_array[c_n,8])
   ...
   # centre=MagCoil(array([0.0,0.0,0.0]), array([0,0,1.0]), R=1.0, I=0)
    centre=MagCoil(array([0.0,0.0,0.0]), array([data_array[c_n,9],data_array[c_n,10],data_array[c_n,11]]), R=1.0, I=0)
...
#    print("coordinate:",smidge)
 ...
 #           print("number of TF :", fil_coil_tot)
            for coil_o in range(fil_coil_tot):
                ...
                #c1=CurrentFilament(fil_ar[coil_o,2],(fil_ar[coil_o,1]))
...
#            print(np.shape(Bnet))
        ...
        #F = (data_array[c_n,2])*(np.cross(Bnet,dl))
...
def fil_coil_forces(B_back,c_n,fil_ar,circ_coil=True,data_array=[0,0,[0,0,0]]): 
 ...
 #   print("coil x,y,z:")
  #  print(data_array[c_n,6],data_array[c_n,7],data_array[c_n,8])
    ...
    #centre=MagCoil(array([0.0,0.0,0.0]), array([0,0,1.0]), R=1.0, I=0)
    centre=MagCoil(array([0.0,0.0,0.0]), array([data_array[0,9],data_array[0,10],data_array[0,11]]), R=1.0, I=0)

    cc=CurrentFilament(fil_ar[c_n,2],(fil_ar[c_n,0]*fil_ar[c_n,1]))
    #cc=CurrentFilament(fil_ar[c_n,2],fil_ar[c_n,1])
 ...
 #       c_n = 10
    ...
    print("cn,point,Fr,dl_x,dl_y,dl_z,nvect,check",file=temp)
    for i in range(num_f-1):
...
#            print(np.shape(Bnet))
        dl = array([x-x_next,y-y_next,z-z_next])
        #Calculate force on coil filament
        #print("F test:",c_n,fil_ar[c_n])
        F = (fil_ar[c_n,0]*fil_ar[c_n,1])*(np.cross(Bnet,dl))
        #F = (fil_ar[c_n,1])*(np.cross(Bnet,dl))
#            print("F per element =", F)
        F_mag = np.sqrt((F[0,0])**2 + (F[0,1])**2 + (F[0,2])**2)
#            print(F_mag)
 #       Fr = (np.cos(angle)*F[0,0]) + ( np.sin(angle)*F[0,1])
   ...
   #     print("F,dl:",F,dl)
        ...
        vect2 = [(p3[0]-p2[0]),(p3[1]-p2[1]),(p3[2]-p2[2])]
        n_vect = np.cross(vect2,vect1)
        dl_m = np.sqrt((dl[0]**2)+(dl[1]**2)+(dl[2]**2))
        c_dl = [dl[0]/dl_m,dl[1]/dl_m,dl[2]/dl_m]
        theta = np.arctan(1/dl_m)
        r = np.sqrt((1**2)+(dl_m**2))

         #2D perp vector calc
        '''
        if n_vect[0] != 0 and n_vect[1]==0 and n_vect[2]==0:
            n_vect = [0,(r*np.sin(theta)+p2[1]),(r*np.cos(theta)+p2[2])]
        elif n_vect[1] != 0 and n_vect[0]==0 and n_vect[2]==0:
            n_vect = [(r*np.sin(theta)+p2[0]),0,(r*np.cos(theta)+p2[2])]
        elif n_vect[2] != 0 and n_vect[1]==0 and n_vect[0]==0:
            n_vect = [(r*np.cos(theta)+p2[0]),(r*np.sin(theta)+p2[1]),0]
        '''
        #3D perp vector calc
        #else:
        ...
        if True:
            ...
            n_vect = [(p1[0]-p2[0]),(p1[1]-p2[1]),(p1[2]-p2[2])]
            n_mag = np.sqrt((n_vect[0]**2)+(n_vect[1]**2)+(n_vect[2]**2))
            c_vect = [n_vect[0]/n_mag,n_vect[1]/n_mag,n_vect[2]/n_mag]
            check = abs(np.dot(c_dl,c_vect))
            ...
            while check > 0.1:
                #TODO: make more efficant 
                point_adjust = add
                add = add + 1
                if point_adjust==num_f:
                    print("add greater than nfil")
                
                if p1[0]==p2[0] and p1[1]==p2[1] and p1[2]==p2[2]:
                    point_adjust = point_adjust + 1
                    print(i,"Equal")

                p2 = array_in[point_adjust]
                n_vect = [(p1[0]-p2[0]),(p1[1]-p2[1]),(p1[2]-p2[2])]
                n_mag = np.sqrt((n_vect[0]**2)+(n_vect[1]**2)+(n_vect[2]**2)) 
                c_vect = [n_vect[0]/n_mag,n_vect[1]/n_mag,n_vect[2]/n_mag]
                ...
                if check < 0:
                    n_vect = [(p2[0]-p1[0]),(p2[1]-p1[1]),(p2[2]-p1[2])] 
                    n_mag = np.sqrt((n_vect[0]**2)+(n_vect[1]**2)+(n_vect[2]**2))
                    c_vect = [n_vect[0]/n_mag,n_vect[1]/n_mag,n_vect[2]/n_mag] 
                    check = np.dot(c_dl,c_vect)
                '''
                    
                
        #x=rho*np.sin(theta)*np.cos(phi) y=rho*np.sin(theta)*np.sin(phi) z=rho*cos(phi)
        n_mag = np.sqrt((n_vect[0]**2)+(n_vect[1]**2)+(n_vect[2]**2))
        mag_check = np.sqrt((c_vect[0]**2)+(c_vect[1]**2)+(c_vect[2]**2))
        
        if n_mag == 0:
            Fr = np.dot(Fc,n_vect)
        else:
            Fr = np.dot(Fc,n_vect)/n_mag
            
        #print(c_n,i,point_adjust,n_vect,p1,p2,n_mag,mag_check,check,sep=',',file=temp)
...
#    print("total length:",dl_sum, "should be:", (2*np.pi*0.13))
    ...
    t_dr = data_array[0,4]/data_array[0,0]
    t_dz = data_array[0,5]/data_array[0,1]
    f_Nr = int(np.sqrt(fil_ar[c_n,0]))
    f_Nz = f_Nr
    f_dr = t_dr * f_Nr
    f_dz = t_dz * f_Nz
    #Fr_sum = Fr_sum *(dl_sum/(2*np.pi*(data_array[0,3]))) #TODO ask why this is
    h_s = hoop_s_calc(Fr_sum,data_array[0,3],f_dz,f_dr)
#    print("Hoop stress in coil:",c_n," =", h_s, "N/m^2, = ",h_s/(10**6),"N/mm^2 = MPa")
...
#SPLIT COIL COORD ARRAY INTO ONE "CHUNK" PER COIL
    ...
    if use_circ_c==True:
        ...
        circ_coil_max = np.size(data_array[:,0])
        coil_num = coil_num + circ_coil_max
#Make pool
...
#Close pool and join results
        ...
        FMag_circ_ar = np.zeros(circ_coil_max)
...
#Make pool
        ...
        coil_num = coil_num + fil_coil_max
        pool = mp.Pool(proc_num)

#Compute B field from each coil
        fil_out = [ pool.apply_async(fil_coil_forces, (B_back,c_n,fil_ar_in,use_circ_c,data_array)) for c_n in range(fil_coil_max)]
        #fil_out = [ pool.apply_async(fil_coil_forces, (B_back,c_n,data_array,use_circ_c,fil_ar_in)) for c_n in range(fil_coil_max)]

#Close pool and join results
        pool.close()
        pool.join()

        FMag_fil_ar = np.zeros(fil_coil_max)
...
#    print("MAX HOOP STRESS:",max_hs*(10**-6),"Mpa")            
    ...
    if use_circ_c==True:
        Tot_F_coil_C = np.zeros((circ_coil_max,7))
        for coil_c in range(circ_coil_max): 
            print(coil_c)
            print("coil num,Fx,Fy,Fz,FMag,Fr,hoop_s(MPa)")
            Tot_F_coil_C[coil_c,0] = out[coil_c].get()[3] # coil num
            Tot_F_coil_C[coil_c,1] = out[coil_c].get()[0][0,0] # Fx 
            Tot_F_coil_C[coil_c,2] = out[coil_c].get()[0][0,1] # Fy 
            Tot_F_coil_C[coil_c,3] = out[coil_c].get()[0][0,2] # Fz 
            Tot_F_coil_C[coil_c,4] = FMag_circ_ar[coil_c] #FMag
            Tot_F_coil_C[coil_c,5] = (out[coil_c].get())[1] #Fr
            Tot_F_coil_C[coil_c,6] = ((out[coil_c].get())[2]) #hoop stress

    if use_fil_c == True:
        Tot_F_coil_F = np.zeros((fil_coil_max,7))
        for coil_c in range(fil_coil_max): 
            print(coil_c)
            print("coil num,Fx,Fy,Fz,FMag,Fr,hoop_s(MPa)")
            Tot_F_coil_F[coil_c,0] = fil_out[coil_c].get()[3] # coil num
            Tot_F_coil_F[coil_c,1] = fil_out[coil_c].get()[0][0,0] # Fx 
            Tot_F_coil_F[coil_c,2] = fil_out[coil_c].get()[0][0,1] # Fy 
            Tot_F_coil_F[coil_c,3] = fil_out[coil_c].get()[0][0,2] # Fz 
            Tot_F_coil_F[coil_c,4] = FMag_fil_ar[coil_c] #FMag
            Tot_F_coil_F[coil_c,5] = (fil_out[coil_c].get())[1] #Fr
            Tot_F_coil_F[coil_c,6] = ((fil_out[coil_c].get())[2]) #hoop stress

#    print("TOTAL RADIAL FORCE:", Fr_tot)
#    print("MAX HOOP STRESS:",max_hs*(10**-6),"Mpa")            


    return(hs_sum,Tot_F_coil_C,Tot_F_coil_F)
...
def pf_sample_coords(coil_ar,nfil): 
    ...
    for i in range(nfil):
        ...
        if count_zeros ==3: 
            ...
            if coil_ar[9] == 1:
                x = coil_ar[6] 
                y = coil_ar[7] + R*sin(angle)
                z = coil_ar[8] + R*cos(angle)            
            elif coil_ar[10] == 1:
                x = coil_ar[6] + R*sin(angle)
                y = coil_ar[7]  
                z = coil_ar[8] + R*cos(angle)
            elif coil_ar[11] == 1:
                x = coil_ar[6] + R*cos(angle)
                y = coil_ar[7] + R*sin(angle) 
                z = coil_ar[8]
            ...
            if angle2 == 0 or angle2 == np.pi:
                rho=R
            change_vector = [] 
            change_vector.append((rho*cos(angle)*v1[0]) + (rho*sin(angle2)*v2[0]))
            change_vector.append((rho*sin(angle)*v1[1]) + (rho*sin(angle2)*v2[1]))
            change_vector.append((rho*cos(angle)*v1[2]) + (rho*cos(angle)*v2[2]))
#        print(change_vector)
